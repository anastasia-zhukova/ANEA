...
from entity_class_annotator.ANEA_annotator.graph.node import Node
from entity_class_annotator.ANEA_annotator.graph.node_properties import WikiProperties, WikiLink, Areas
...
class Graph:
    ...
    def __init__(self, selected_terms, fraction=TERM_GROUPS, domain=None):
        self.graph_top_down = {}
        self.graph_down_top = {}
        ...
        self.not_in_graph = {}
        self.terms = selected_terms
        self.fraction = fraction
        self.domain = domain
        self.num_freq_gr_sel = None
        self.num_freq_gr_all = None

        self.model = None

        self._create_graph(selected_terms)
    ...
    def _create_graph(self, selected_terms):
        ...
        for key, term in selected_terms.items():
            ...
            if term.wikt_page_head not in terms_grouped:
                terms_grouped[term.wikt_page_head] = []
            terms_grouped[term.wikt_page_head].append(term)

        terms_grouped = {k:v for k,v in sorted(terms_grouped.items(), reverse=True,
                                               key=lambda x: (len(x[1]), sum([v.counter for v in x[1]])))}
        self.num_freq_gr_sel =  int(len(terms_grouped) / self.fraction) if self.fraction is not None else len(terms_grouped)
        self.num_freq_gr_all = len(terms_grouped)

        print("All groups of terms: {0}, selected for the graph: {1}".format(str(len(terms_grouped)),
                                                                             str(self.num_freq_gr_sel)))

        widgets = [progressbar.FormatLabel(
            "PROGRESS: Processing %(value)d-th/%(max_value)d (%(percentage)d %%) groups of terms (in: %(elapsed)s).")]
        bar = progressbar.ProgressBar(widgets=widgets, maxval=len(terms_grouped)).start()

        for j, (head_link, terms) in enumerate(terms_grouped.items()):
            # skip terms w/o wiki-head as candidates for core terms
            if head_link is None:
                continue

            if self.fraction is not None:
                if j > len(terms_grouped)/self.fraction:
                    break
